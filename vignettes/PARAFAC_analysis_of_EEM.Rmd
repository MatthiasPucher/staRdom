...
For easy data correction and calculations of peaks, indices calculation and slope parameters without using R functions and codes please see [vignette for basic analysis](Basic_analysis_of_DOM_samples.html). For information on eemR and its functions please see the [eemR vignette](https://CRAN.R-project.org/package=eemR). Details on the actual PARAFAC calculation can be found in the [multiway documentation](https://CRAN.R-project.org/package=multiway).
...
EEM data import is done with `eem_read` (package eemR). Currently you can use it to import from Cary Eclipse, Aqualog, Fluoromax-4 and Shimadzu instruments.
...
Dilution data is read using the base R function read.table. Any function providing a variable of class `data.table` can be used. Here, we read in a table containig meta data provided with the package as an example.
...
## Check data
The data can be checked for possible incorrect entries, such as NAs ([‘Not Available’ / missing values](https://stat.ethz.ch/R-manual/R-devel/library/base/html/NA.html)) in data, duplicate and invalid sample names, wavelength range mismatches, missing data, inconsistencies in samples names between EEM, absorbance and metadata (e.g. samples missing in one of the three sets) and missing correction methods. In the case of problems, you need to reorganise your data manually and revise the steps mentioned above. The `eem_checkdata` function can be run any time during the process to see if any changes in the dataset caused inconsistencies.
...
The above mentioned absence of absorbance data for the nano sample can be justified, as absorbance of MilliQ is nearly 0 over the complete spectrum.
...
Fluorescence is normalised to a standard scale of Raman Units by dividing all intensities by the area of the Raman peak of a MilliQ sample. It allows for the comparability of data from different fluorometers or different settings on the same fluorometer. Depending on where you get the data from, you can use blanks, numeric values or data frames as source for the values.
...
From this step onwards, blanks are not needed anymore. You can remove them from your sample set.
...
If wavelength ranges needed for certain indices or peaks an interpolation is done automatically (no extrapolation!). Usually according warnings can be ignored. 
...
`abs_parms` can be used to calculate a254, a300, E2:E3, E4:E6, S275-295, S350-400, S300-700, SR and the wavelength distribution of absorption spectral slopes `r citep(list("10.4319/lo.2008.53.3.0955","10.1016/j.marchem.2004.02.008",citation(package="cdom"),"10.4319/lo.2009.54.2.0590"))`.
...
Finding an appropriate PARAFAC model is an iterative process. For comparing different methods, the same dataset and way of analysis were used as in the drEEM tutorial which is an appendix to `r citet("10.1039/c3ay41160e")`.

## Loading data

The package comes with some example data but to keep the package size low, all data below 250 nm excitation and above 580 nm emission were removed and only 15 samples of the original set are part of the package. The complete sample set is available in the drEEM toolbox for MATLAB (http://models.life.ku.dk/drEEM). The example PARAFAC models are calculated based on the complete sample set. Because of that limitation, some calculations are described but cannot be repeated with the provided example data.
...
We recommend to use `eem_checkdata` again before continuing the further analysis!
...
* `eem_red2smallest` removes data in EEMs that is not present in all samples. This is important for a PARAFAC analysis because the algorithm needs matrices of similar size and "shape".
* `eem_exclude` removes data from the sample set, provided by a list of outliers. This list can be created manually or by `eem_leverage_ident`.
...
Sample "667sf" will be used to show this process graphically. To extract the sample the [regular expression](https://stat.ethz.ch/R-manual/R-devel/library/base/html/regex.html) "^667sf$" is used. ^ stands for the beginning of the string and \$ for the end.
...
PARAFAC modes can be rescaled so that their maximum is 1 and effects are more visible in the plots. The rescaling is corrected in the A mode (sample mode).

In case of uneven peak heights, `eempf_rescaleBC` can help to improve the visibility of your graphs. The parameter `newscale` specifies the root mean-squared error of each column in matrices B and C. This is compensated in the A-mode (sample loadings). Alternatively `newscale` can be set `"Fmax"`, each peak has a height of 1 then.
...
To see the results of the model created from the whole sample set, you can use the generated PARAFAC model included in the package.
```{r include=TRUE}
data(pf_models)
```
...
In order to save the decisions of the graphical outlier determination with `eempf_leverage_ident`, we advise to generate the exclude list manually. See example below.
...
The outliers determined above are included to show the difference in residuals. Analysing these residuals can show deficits in model creation, problems with sample handling and lab equipment or it can already be helpful in answering scientific questins.
...
Due to long calculation times with higher accuracy in the model calculation, the tolerance is only increased in the last step.
...
As a way of model validation the core consistency can be calculated. Please see `r citet("10.1039/c3ay41160e")` for the suitability of the core consistency for model validation.
