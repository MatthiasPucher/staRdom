...
* calculating common absorbance (slope) parameters:
    ...
    * absorbance at 300 nm (a300; `r citet("10.1007/s00027-005-0754-9")`),
...
If your instrument is not amongst the mentioned ones, eemR offers the opportunity to [create custom import functions](https://cran.r-project.org/web/packages/eemR/vignettes/custom-import-function.html).
...
```{r eval=TRUE, fig.width=7, fig.height = 6, message=FALSE, warning=FALSE, include=TRUE, paged.print=TRUE}
...
```
```{r eval=TRUE, fig.width=7, fig.height = 6, message=FALSE, warning=FALSE, include=TRUE}
eem_overview_plot(eem_list, spp=9, contour = TRUE)
```
...
Finding an appropriate PARAFAC model is an iterative process. For comparing different methods, we used the same dataset and analysis procedure as in the drEEM tutorial which is a supplement to `r citet("10.1039/c3ay41160e")`.
