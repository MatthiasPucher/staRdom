...
output: 
  html_document:
    ...
    code_folding: hide
---
This report was created using the R package staRdom version `r packageVersion("staRdom")`, (`r packageDescription("staRdom")$Author`, `r unlist(strsplit(packageDescription("staRdom")$Date,"-"))[1]`)
...
#
# This is the only parameter that has to be set!
output_dir = "C:/some_folder/output/" # e.g. output_dir = "C:/some_folder/output/"
...
sample_dir = system.file("extdata/cary/scans_day_1", package = "eemR") # e.g. sample_dir = "C:/some_folder/input/fluor/"
...
absorbance_dir = system.file("extdata/absorbance_eemR",package = "staRdom") # e.g. absorbance_dir = "C:/some_folder/input/absorbance/"
...
metadata = system.file("extdata/metatable_eemR.csv",package = "staRdom") # e.g. metadata = "C:/some_folder/input/metatable.csv"
...
eem_list <- eem_read(sample_dir,recursive=TRUE)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
if(metadata != FALSE){
  meta <- read.table(metadata, header = TRUE, sep = meta_sep_dec[1], dec = meta_sep_dec[2], row.names = col_samples)
}

### Read absorbance data ###
#~~~~~~~~~~~~~~~~~~~~~~~~~~#
if(absorbance_dir != FALSE){
  abs_data <- absorbance_read(absorbance_dir,sep = abs_sep_dec[1], dec = abs_sep_dec[2])
}
...
# It is a Hitachi related string.
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
if(absorbance_dir != FALSE){
  if(absorbance_cuv_len == "meta") cuvl <- meta[col_cuv_len] else cuvl <- absorbance_cuv_len
...
if(interpolation) eem_list <- eem_list %>% eem_interp()
...
if(absorbance_dir != FALSE){
  indices_peaks <- indices_peaks %>%
    full_join(abs_parms(abs_data,cuvl), by="sample")
}
...
if(data_file != FALSE){
  save(list=c(eem_name),file = paste0(output_dir,data_file,"_",format(Sys.time(), "%Y%m%d_%H%M%S"),".RData"))
}
```
# Sample overview
...
if(overview | output_overview_png) {
  ov_plots <- eem_list %>%
    eem_overview_plot(spp = overview_number)
}
...
if(output_single_png | single_plots){
  sing_plots <- lapply(eem_list, ggeem, fill_max = fill_max)
}
...
if(output_single_png){
  lapply(sing_plots, function(pl){
    png(paste0(output_dir,"/EEM_single_",format(Sys.time(), "%Y%m%d_%H%M%S"),'.png',sep=""),width=1000,height=1000,units = "px", pointsize = 25)
    plotp <- pl + theme(text = element_text(size=35))
    print(plotp)
    dev.off()
  })
